...
function process_measurements(
    ...
    process_all_equal_time_measurements::Bool = false,
    ...
    if export_to_csv && iszero(MPI.Comm_rank(comm))
        ...
        h5open(h5filename, "r") do H5File
            ...
            if process_all_equal_time_measurements
                for correlation in keys(H5File["CORRELATIONS"]["EQUAL-TIME"])
...
function process_measurements(;
    # KEYWORD ARGUMENTS
    datafolder::String,
    parallelize_rebinning::Bool = true, # not used here
    n_bins::Union{Int, Nothing} = nothing,
    ...
    if isempty(pIDs)
        bin_dir = joinpath(datafolder,"sim_bins")
