```@meta
EditURL = "../../../examples/hubbard_ladder_mpi.jl"
```

# Hubbard Ladder with MPI Parallelization
Download this example as a [Julia script](../assets/scripts/examples/hubbard_ladder_mpi.jl).

This is a ready-to-run script for simulating a two-dimensional (2D) Hubbard model
on a rectangular lattice goemetry (ladder) with a stripe wavefunction using MPI parallelization.
