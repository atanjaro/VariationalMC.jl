...
# 1) Hubbard Chain with an Antiferromagnetic Wavefunction
Download this example as a [Julia script](../assets/scripts/tutorials/hubbard_chain.jl).

This tutorial will work through simulating a repulsive Hubbard model on a one-dimensional (1D) chain
using a wavefunction with Néel antiferromagnetic order.
...
electron on site $i$ in the lattice, and ``\hat{n}_{\sigma,i} = \hat{c}^\dagger_{\sigma,i} \hat{c}^{\phantom \dagger}_{i,\sigma}``
...
We also initialize the Canonical Ensemble particle configuration that lives on the lattice. In this case, given the number of spin-up and spin-down particles that we
want to live on the lattice, [`ParticleConfiguration`](@ref) will determine the correct density.
...
add the `"spin-z"` parameter for optimization.
...
```
> julia hubbard_chain.jl 1 8 1.0 4 4 1.0 false 200 3000 6000 300 600
```
runs a VMC simulation of a $N = 8$ 1D Hubbard model at half-filling (4 spin-up, 4 spin-down)
with interaction strength $U = 1.0$. Since there there are no pairing terms in the wavefunction,
we set `ph_transform = false`.
